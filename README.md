...
Neither `LndWallet` nor `ClnWallet` can hand out a scalar from the node's seed over REST, so roux ships `FileSecretProvider(path)`: HKDF-SHA256 over a 32-byte seed file, created with mode 0600 on first use and refused if anything but its owner can read it (a POSIX check; on Windows the mode bits say nothing about the ACL, so restrict the file yourself). That file is the wallet file now, and it is only half of the pair: losing the seed or losing the records leaves an unresolved swap unclaimable, so back them up together. A host whose wallet already derives from a seed of its own should implement the three methods over that instead.
